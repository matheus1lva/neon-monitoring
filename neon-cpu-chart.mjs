// Neon usage dashboard. Run: bun neon-cpu-chart.mjs  ->  open http://localhost:8787
// API key lives in browser localStorage; this server only proxies it to Neon
...
// The live per-minute CPU/RAM/connections charts in the Neon console come from an
// internal monitoring endpoint authed by the browser session, NOT by an API key.
// To add those, capture that request (DevTools > Network > Copy as cURL) and wire
// it into the /api/metrics route below.
